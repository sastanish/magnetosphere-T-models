module reconnection
  ...
  !! Derived Quantities !!
  real(8), dimension(:,:,:), allocatable :: Fx,Fy,Fz,Jx,Jy,Jz,Bfx,Bfy,Bfz
  real(8), dimension(:,:,:), allocatable :: lambda,alpha,omega1,omega2,magB,magF

  !! Control !!
  integer :: compute_level=0

contains

  subroutine compute_aux_variables()
    ...
    do i=2,nx-1
      ...
      ! J = Curl(B)
      Jx(i,j,k) = dy(Bz(i,j,k)) - dz(By(i,j,k))
      Jy(i,j,k) = dz(Bx(i,j,k)) - dx(Bz(i,j,k))
      Jz(i,j,k) = dx(By(i,j,k)) - dy(Bx(i,j,k))
      ! F = u x B
  ...
  end subroutine compute_aux_variables
  ...
  function dx(A,i,j,k) result(C)
    real(8), dimension(:,:,:), intent(in) :: A
    integer, intent(in) :: i,j,k
    real(8), intent(out) :: C
    C = (A(i+1,j,k) - A(i-1,j,k)/(x(i+1)-x(i-1))
  end function dx

  function dy(A,i,j,k) result(C)
    real(8), dimension(:,:,:), intent(in) :: A
    integer, intent(in) :: i,j,k
    real(8), intent(out) :: C
    C = (A(i,j+1,k) - A(i,j-1,k)/(y(i+1)-y(i-1))
  end function dy

  function dz(A,i,j,k) result(C)
    real(8), dimension(:,:,:), intent(in) :: A
    integer, intent(in) :: i,j,k
    real(8), intent(out) :: C
    C = (A(i,j,k+1) - A(i,j,k-1)/(z(i+1)-z(i-1))
  end function dz
