...
contains
  ...
  subroutine compute_c2_t1(x,y,z,bx,by,bz,fx,fy,fz,bfpx,bfpy,bfpz,lambda,&
    ...
    mag = sqrt(c2_t1_x**2 + c2_t1_y**2 + c2_t1_z**2)
  ...
  subroutine compute_c2_t2(x,y,z,bx,by,bz,fx,fy,fz,bfpx,bfpy,bfpz,lambda,alpha,&
    ...
    mag = sqrt(c2_t2_x**2 + c2_t2_y**2 + c2_t2_z**2)
  ...
  subroutine compute_total_rate(x,y,z,bx,by,bz,mag)

    real(8), intent(in), dimension(:) :: x,y,z
    real(8), intent(in), dimension(:,:,:) :: bx,by,bz
    real(8), allocatable, dimension(:,:,:) :: fx,fy,fz
    real(8), allocatable, dimension(:,:,:) :: jx,jy,jz
    real(8), allocatable, dimension(:,:,:) :: bfpx,bfpy,bfpz
    real(8), allocatable, dimension(:,:,:) :: alpha,lambda

    real(8), intent(out), dimension(:,:,:) :: mag

    integer :: ix, iy, iz, r(3,3)
    real(8) :: hx, hy, hz
    real(8) :: curl_bfpx, curl_bfpy, curl_bfpz
    real(8) :: dxa, dya, dza, omega1, omega2, glamb_B
    real(8), dimension(:,:,:), allocatable :: magF, magB, rx, ry, rz
    integer nx,ny,nz
    ...
    allocate(magF(nx,ny,nz))
    allocate(magB(nx,ny,nz))
    allocate(rx(nx,ny,nz))
    allocate(ry(nx,ny,nz))
    allocate(rz(nx,ny,nz))
    ...
    allocate(bfpx(nx,ny,nz))
    allocate(bfpy(nx,ny,nz))
    allocate(bfpz(nx,ny,nz))
    allocate(alpha(nx,ny,nz))
    allocate(lambda(nx,ny,nz))


    call compute_forces(x,y,z,bx,by,bz,jx,jy,jz,fx,fy,fz)
    call compute_parameters(x,y,z,bx,by,bz,jx,jy,jz,fx,fy,fz,&
                                bfpx,bfpy,bfpz,alpha,lambda)
    ...
    do iz = 1,nz
      ...
      do iy = 1,ny
        ...
        do ix = 1,nx
          ...
          ! Grad(lambda) dot B
          glamb_B = (lambda(r(3,1),r(2,2),r(2,3)) - lambda(r(1,1),r(2,2),r(2,3)))/(2*hx)*bx(ix,iy,iz) &
                  + (lambda(r(2,1),r(3,2),r(2,3)) - lambda(r(2,1),r(1,2),r(2,3)))/(2*hy)*by(ix,iy,iz) &
                  + (lambda(r(2,1),r(2,2),r(3,3)) - lambda(r(2,1),r(2,2),r(1,3)))/(2*hz)*bz(ix,iy,iz)
          ...
          ! Curl(B_fp) dot F/|F|
          omega1 = (curl_bfpx*fx(ix,iy,iz) + curl_bfpy*fy(ix,iy,iz) + curl_bfpz*fz(ix,iy,iz))/magF(ix,iy,iz)

          ! Curl(B_fp) dot B_fp/|B|^2
          omega2 = (curl_bfpx*bfpx(ix,iy,iz) + curl_bfpy*bfpy(ix,iy,iz) + curl_bfpz*bfpz(ix,iy,iz))/(magB(ix,iy,iz)**2)

          rx = ((lambda*omega1 - glamb_B) * Fx(ix,iy,iz)/magF(ix,iy,iz) &
                + lambda*(alpha+omega2) * bfpx(ix,iy,iz) &
                + (dya*bz(ix,iy,iz)-dza*by(ix,iy,iz)) )

          ry = ((lambda*omega1 - glamb_B) * Fy(ix,iy,iz)/magF(ix,iy,iz) &
                + lambda*(alpha+omega2) * bfpy(ix,iy,iz) &
                + (dza*bx(ix,iy,iz)-dxa*bz(ix,iy,iz)) )

          rz = ((lambda*omega1 - glamb_B) * Fz(ix,iy,iz)/magF(ix,iy,iz) &
                + lambda*(alpha+omega2) * bfpz(ix,iy,iz) &
                + (dxa*by(ix,iy,iz)-dya*bx(ix,iy,iz)) )

        end do
      end do
    end do

    mag = (1/magB) * sqrt(rx**2 + ry**2 + rz**2)
