...
def find_magnetosphere_library():
  ...
  if os.name == 'posix': # Linux or macOS
    if os.uname().sysname == 'Darwin': # macOS
      lib_path = install_path + f'/lib{lib_name}.dylib'
    else: # Linux
      lib_path = install_path + f'/lib{lib_name}.so'
  elif os.name == 'nt': # Windows
      lib_path = install_path + f'/{lib_name}.dll'
  else:
...
def ta16(date,data,x,y,z):
  ...
  api = find_magnetosphere_library()
