program main
  ...
  !Data vars
  real(8), dimension(:,:,:), allocatable :: Bx, By, Bz, rate, lrate, wrate, arate
  real(8), dimension(:,:), allocatable :: pressure
  real(8), dimension(:), allocatable :: x,y,z,xr,yr,zr,out_rate,out_lrate,out_wrate,out_arate,out_pos
  ...
  allocate(out_lrate(start_ind:end_ind))
  allocate(out_wrate(start_ind:end_ind))
  allocate(out_arate(start_ind:end_ind))
  ...
  do f = start_ind,end_ind
    ...
    call load_rates_from_netcdf(trim(dir)//'rates_'//trim(adjustl(str_ind))//'.nc',xr,yr,zr,rate,lrate,wrate,arate)
    deallocate(xr)
    deallocate(yr)
    deallocate(zr)
    ...
    out_lrate(f) = lrate(xind,yind,zind)
    out_wrate(f) = wrate(xind,yind,zind)
    out_arate(f) = arate(xind,yind,zind)
  ...
  open(newunit=outfile, file=trim(dir)//"x-point_location_for_all_rates.lst", status="new")
  open(newunit=omnifile, file=trim(dir)//"input_data.lst", status="old")
  write(outfile,"(A)") "X-point locations."
  write(outfile,"(A)") "FORMAT:"
  write(outfile,"(A)") "YEAR    DAY    HOUR    MIN    TOTAL_RATE   LORRENZ_RATE    WINDING_RATE    ALIGNED_RATE    DISTANCE"
  ...
  do i = start_ind,end_ind
    ...
    write(outfile,'(I4, 4x, I3, 4x, I4, 4x, I3, 4x, f10.4, 4x, f10.4, 4x, f10.4, 4x, f10.4, 4x, f6.3)') year, day, hour, minn, out_rate(i), out_lrate(i), out_wrate(i), out_arate(i), out_pos(i)
  end do
  close(outfile)
  close(omnifile)

  deallocate(out_rate)
  deallocate(out_lrate)
  deallocate(out_wrate)
  deallocate(out_arate)
  deallocate(out_pos)
...
subroutine load_rates_from_netcdf(filename,x,y,z,rate,lrate,wrate,arate)

  use netcdf

  character(*), intent(in) :: filename
  real(8), allocatable, dimension(:,:,:), intent(inout) :: rate, lrate, wrate, arate
  real(8), allocatable, dimension(:), intent(inout) :: x,y,z
  ...
  allocate(lrate(nx,ny,nz))
  allocate(wrate(nx,ny,nz))
  allocate(arate(nx,ny,nz))
  ...
  call check(nf90_get_var(nc_id,5,lrate))
  call check(nf90_get_var(nc_id,6,wrate))
  call check(nf90_get_var(nc_id,7,arate))

  call check(nf90_close(nc_id))

end subroutine load_rates_from_netcdf
