...
for i,line in enumerate(kp_data):
    kp[i] = line[7]
    time = pd.to_datetime(str(line[0]) + "/" + str(line[1]) + "/" + str(line[2]) + ' ' + str(line[3]), format="%Y/%m/%d %H.0")
    times.append(time)
...
# Now we graph
fig, (rax1, iax, kax) = plt.subplots(figsize=(10,12),nrows=3)
...
dplot = iax.plot(dst_times,dst,label="DST",color="tab:blue")
splot = sax.plot(rate_dtimes,symh,label="Symh",color="tab:red")

kplot = kax.bar(kp_times,kp,label="Kp")
