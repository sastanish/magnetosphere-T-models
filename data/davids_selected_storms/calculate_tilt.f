...
c
      ...
      do I = 1,IARGC() ! Loop through command line args
        CALL getarg(I,NAME)
        CALL CALCULATE_TILT(NAME)
      end do
      END

c  Calculate tilt given filename
      SUBROUTINE CALCULATE_TILT (NAME)
      implicit real*8 (a-h,o-z)
      REAL*8 TILT,VXX,VYY,VZZ

      CHARACTER*90 NAMEIN,NAMEOUT,NAME

      REAL*8 BX,BY,BZ,VX,VY,VZ,V_SW,T,DEN,PA,BXX,BYY,BZZ
      INTEGER IYEA,IDA,IHOU,MI,IMFLAG,SWFLAG,AeIND,SymH

      COMMON /GEOPACK1/ AAA(15),TILT,BBB(18)

      NAMEIN=trim(NAME)//'_filled_gaps.lst'
      NAMEOUT=trim(NAME)//'_filled_gaps_with_tilt.lst'

      OPEN (UNIT=1,FILE='data/'//NAMEIN,STATUS='OLD')     !  filename for the solar wind/IMF/Sym-H data
 505  FORMAT(2I4,2I3,3F8.2,3F8.1,F7.2,F9.0,F6.2,2I6,2I3)

      OPEN (UNIT=3,FILE='data/'//NAMEOUT)
 444  FORMAT(2I4,2I3,3F8.2,3F8.1,F7.2,F9.0,F6.2,2I6,2I3,F8.4,F7.2)

 1    READ (1,505,END=2) IYEA,IDA,IHOU,MI,BX,BY,BZ,VX,VY,VZ,DEN,T,PA,
     * AeIND,SymH,IMFLAG,SWFLAG

      V_SW=DSQRT(VX**2+VY**2+VZ**2)

C     Derived Parameters
      Pdyn=1.937D-6*DEN*V_SW**2
C      CALL RECALC_08 (IYEA,IDA,IHOU,MI,0,-400.,0.,0.)    !1st step: call RECALC_08 with
      CALL RECALC_08 (IYEA,IDA,IHOU,MI,0,VX,VY,VZ)       !3rd step: call RECALC_08 with
c                                                          ! an actual SW direction VX, VY,VZ
      CALL GSWGSE_08 (BXX,BYY,BZZ,BX,BY,BZ,-1)        !4th step: convert GSE to GSW
      WRITE(3,444) IYEA,IDA,IHOU,MI,BXX,
     * BYY,BZZ,VX,VY,VZ,DEN,
     * T,PA,AeIND,SYMH,IMFLAG,SWFLAG,
     * TILT,Pdyn
