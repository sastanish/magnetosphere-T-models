program main
  ...
  do f = start_ind,end_ind
    ...
    call load_rate_from_netcdf(trim(dir)//'hall_rate_'//trim(adjustl(str_ind))//'.nc',xr,yr,zr,rate)
    deallocate(xr)
    deallocate(yr)
    deallocate(zr)
  ...
  open(newunit=outfile, file=trim(dir)//"x-point_location_hall.lst", status="new")
